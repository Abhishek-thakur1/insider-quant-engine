...
export class LiquidityTrapDetector implements IDetector {
	...
	public async analyze(liveTick: TickData): Promise<void> {
		...
		if (history.length >= 10) {
			// Need at least 10 mins of context
			...
			const isBearishRejection =
				upperWickPct >= MIN_WICK_PCT && closedCandle.close < closedCandle.open

			if (
				sweptHighs &&
				closedBelowHighs &&
				isBearishRejection &&
				closedCandle.volume > avgVol * VOL_MULTIPLIER
			) {
			...
			const isBullishRejection =
				lowerWickPct >= MIN_WICK_PCT && closedCandle.close > closedCandle.open
