...
export class VolatilityContraction extends BaseDetector {
    ...
    async analyze(liveTick: TickData): Promise<void> {
        ...
        if (!this.currentCandle) {
            this.currentCandle = { high: liveTick.price, low: liveTick.price, close: liveTick.price, volume: liveTick.volume, startTs: now };
            return;
        }
        ...
        const getAvgRange = (candles: Candle5M[]) => candles.reduce((sum, c) => sum + (c.high - c.low), 0) / candles.length;
        const getAvgVol = (candles: Candle5M[]) => candles.reduce((sum, c) => sum + c.volume, 0) / candles.length;
