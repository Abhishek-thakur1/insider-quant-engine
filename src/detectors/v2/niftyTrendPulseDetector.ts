...
const MIN_BODY_PCT = 0.10                 // real body filter (same)
const CONFIRMS_NEEDED = 2                 // 2 closes in same direction (vs 3 old = too slow)
const VWAP_DIST_PCT = 0.20               // 0.20% from VWAP (vs 0.15% old — passes JSFilter better)
...
export class NiftyTrendPulseDetector implements IDetector {
    ...
    public async analyze(liveTick: TickData): Promise<void> {
        ...
        const isVolConfirmed = this.avgVolume > 0 &&
            current.volume >= this.avgVolume * VOL_CONFIRM_MULT
        ...
        if (allBullish && hasHH && isAboveVwap && bodyPct >= MIN_BODY_PCT) {
            ...
            const volNote = isVolConfirmed ? `${(current.volume / this.avgVolume).toFixed(1)}× vol ✅` : `vol light ⚠️`
            ...
            sendTelegramAlert({
                ...
                volumeSpikeRatio: this.avgVolume > 0 ? Number((current.volume / this.avgVolume).toFixed(1)) : 1,
