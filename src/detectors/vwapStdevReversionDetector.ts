...
export class VwapStdevReversionDetector implements IDetector {
    ...
    public async analyze(liveTick: TickData): Promise<void> {
        ...
        if (this.history.length > ROLLING_WINDOW) this.history.shift()
        ...
        const avgVol = baselineCandles.length > 0
        ...
        const isVolumeConfirmed = isIndex || (avgVol > 0 && closedCandle.volume > avgVol * VOL_MULTIPLIER)
        ...
        if (isOverbought && isBearishRejection && isVolumeConfirmed && isBlockSized && marketBias !== 'bullish') {
            ...
            if (isIndex) console.log(`   Routing to Strike: ${bestStrike?.strike} PE | Premium: ₹${bestStrike?.ltp}`)
        ...
        if (isOversold && isBullishRejection && isVolumeConfirmed && isBlockSized && marketBias !== 'bearish') {
            ...
            if (isIndex) console.log(`   Routing to Strike: ${bestStrike?.strike} CE | Premium: ₹${bestStrike?.ltp}`)
