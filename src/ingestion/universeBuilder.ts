...
export const buildUniverse = async (): Promise<void> => {
    ...
    try {
        ...
        response.data
            .pipe(csv([
                'fytoken', 'name', 'instrument_type', 'lot', 'tick', 'isin',
                'trad_ses', 'last_upd', 'expiry_dt', 'symbol', 'exchange',
                'segment', 'script_code', 'short_sym', 'strike', 'opt', 'fytoken2'
            ]))
            .on('data', (row: any) => {
            ...
            .on('end', () => {
                ...
                console.log(`[Universe Builder] ✅ Successfully built universe with ${universe.length} EQ stocks.`);
